...
class ConvGRUCell(nn.Module):
    ...
    def forward(self,input_tensor,hidden):
        if hidden is None:
           size_h    = [input_tensor.data.size()[0],self.hidden_size] + list(input_tensor.data.size()[2:])
           if self.cuda_flag  == True:
              hidden    = Variable(torch.zeros(size_h)).cuda() 
           else:
              hidden    = Variable(torch.zeros(size_h))
        c1           = self.ConvGates(torch.cat((input_tensor,hidden),1))
        (rt,ut)      = c1.chunk(2, 1)
        reset_gate   = f.sigmoid(rt)
        update_gate  = f.sigmoid(ut)
        gated_hidden = torch.mul(reset_gate,hidden)
        p1           = self.Conv_ct(torch.cat((input_tensor,gated_hidden),1))
        ct           = f.tanh(p1)
...
class ConvGRU(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(ConvGRUCell(input_size=cur_input_dim,
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
           ...
           # h = hidden_state[layer_idx]
            output_inner = []
            for t in range(seq_len):
                h = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :],hidden=hidden_state)
                output_inner.append(h)
